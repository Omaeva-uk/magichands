            ...
            <div className="grid gap-7 grid-cols-3 max-md:grid-cols-1 mt-24 max-lg:mt-5">
                {
                    servicesData.serviceCards.map(item => (
                        <Link href={item.href} data-aos="fade-in" key={item.id} className={` ${item.id === 1 ? 'bg-green' : 'bg-lighGreen'} hover:border-green flex flex-col hover:border-2 transition-all p-6 rounded-3xl `}>
                            <div className="items-start flex max-xl:flex-col gap-3 justify-between">
                                <Image src={item.img} width={70} height={70} alt="Magichands Services Image" />
                                <h3 className="font-raleway self-end text-right text-gray-800 font-medium max-w-[300px] mt-5 mb-3 text-xl">{item.title}</h3>
                            </div>
